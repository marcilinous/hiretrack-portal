...
(function () {
  ...
  function installSbRestShim(sb, opts) {
    ...
    function token() {
      const s = readSession();
      return s && s.access_token ? s.access_token : ANON;
    }
    function authHeaders(extra) {
      return Object.assign(
        { apikey: ANON, Authorization: 'Bearer ' + token(), Accept: 'application/json' },
        extra || {}
      );
    }
    ...
    function from(table) {
      ...
      async function exec() {
        ...
        try {
          ...
          if (resp.status === 401) {
            ...
            if (ns) {
              headers.Authorization = 'Bearer ' + ns.access_token;
              resp = await fetch(reqUrl, fetchOpts);
            }
          }
        } catch (e) {
        ...
        if (!resp.ok) {
          const err =
            payload && typeof payload === 'object'
              ? {
                  message: payload.message || payload.error || resp.statusText,
                  details: payload.details,
                  hint: payload.hint,
                  code: payload.code || String(resp.status),
                }
      ...
      const b = {
        select(cols, o) {
          st.cols = cols == null ? '*' : cols;
          if (st.method !== 'GET') st.want = true;
          if (o && o.count) st.prefer.push('count=' + o.count);
          return b;
        },
        insert(rows) {
          st.method = 'POST';
        ...
        upsert(rows, o) {
          st.method = 'POST';
          st.body = rows;
          st.prefer.push('resolution=merge-duplicates');
          if (o && o.onConflict) st.filters.push('on_conflict=' + o.onConflict);
          return b;
        },
        update(obj) {
          st.method = 'PATCH';
        ...
        eq(c, v) {
          st.filters.push(c + '=eq.' + enc(v));
          return b;
        },
        neq(c, v) {
          st.filters.push(c + '=neq.' + enc(v));
          return b;
        },
        gt(c, v) {
          st.filters.push(c + '=gt.' + enc(v));
          return b;
        },
        gte(c, v) {
          st.filters.push(c + '=gte.' + enc(v));
          return b;
        },
        lt(c, v) {
          st.filters.push(c + '=lt.' + enc(v));
          return b;
        },
        lte(c, v) {
          st.filters.push(c + '=lte.' + enc(v));
          return b;
        },
        like(c, v) {
          st.filters.push(c + '=like.' + enc(v));
          return b;
        },
        ilike(c, v) {
          st.filters.push(c + '=ilike.' + enc(v));
          return b;
        },
        is(c, v) {
          st.filters.push(c + '=is.' + (v === null ? 'null' : v));
          return b;
        },
        in(c, arr) {
          st.filters.push(c + '=in.(' + (arr || []).map((x) => enc(x)).join(',') + ')');
          return b;
        },
        contains(c, v) {
          const val = Array.isArray(v)
            ...
            : typeof v === 'object'
              ...
              : enc(v);
          st.filters.push(c + '=cs.' + val);
          return b;
        },
        not(c, op, v) {
          st.filters.push(c + '=not.' + op + '.' + (v === null ? 'null' : enc(v)));
          return b;
        },
        or(f) {
          st.filters.push('or=(' + f + ')');
          return b;
        },
        match(obj) {
          Object.keys(obj || {}).forEach((k) => st.filters.push(k + '=eq.' + enc(obj[k])));
          return b;
        },
        filter(c, op, v) {
          st.filters.push(c + '=' + op + '.' + enc(v));
          return b;
        },
        order(c, o) {
          const asc = !o || o.ascending !== false;
          st.order.push(c + '.' + (asc ? 'asc' : 'desc'));
          return b;
        },
        limit(n) {
          st.limit = n;
        ...
        then(res, rej) {
          ...
          return exec().catch(rej);
        },
        finally(fn) {
          return exec().finally(fn);
        },
      };
      return b;
    }
    ...
    async function rpc(fn, params) {
      try {
        const resp = await fetch(REST + '/rpc/' + fn, {
          method: 'POST',
        ...
        return result(null, { message: e.message, code: 'fetch_error' });
      }
    }
  ...
  try {
    window.installSbRestShim = installSbRestShim;
  } catch (e) {}
})();
